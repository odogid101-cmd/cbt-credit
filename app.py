...
def post_comment():
    ...
    print("HEADERS:", dict(request.headers))  # ← And this
    
    uid = data.get("user_id")
    text = data.get("text","").strip()
    
    if not uid or not text:
        print(f"MISSING: uid={uid}, text={text}")  # ← And this
        return jsonify({"error": "Missing fields"}), 400
    ...
    if not all([qid, uid, text]): return jsonify({"error": "Missing fields"}), 400
    conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO question_comments (question_id, user_id, comment_text, parent_comment_id) VALUES (%s,%s,%s,%s) RETURNING comment_id", (qid, uid, text, pid))
...
def report_question():
    ...
    try:
        ...
        conn.commit(); return jsonify({"message": "Report submitted"}), 201
    finally: cur.close(); conn.close()
