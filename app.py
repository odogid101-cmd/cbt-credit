...
def admin_required(f):
    ...
    def wrapper(*args, **kwargs):
        # Allow preflight browser CORS requests to pass smoothly
        if request.method == "OPTIONS":
            return "", 200
            
        # Read the custom authentication tracking header
...
def admin_reply_comment():
    ...
    try:
        ...
        return jsonify({"message": "Administrative reply successfully broadcasted"}), 201
    finally:
        cur.close()
        conn.close()
